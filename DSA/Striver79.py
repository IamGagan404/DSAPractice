...
# Find the repeating and missing numbers [1,n]
def rep_mis(nums):
    hash_arr = [0 for x in range(len(nums)+1)]
    print(hash_arr)
    for a in nums:
        hash_arr[a] += hash_arr[a] + 1
    print(hash_arr)
    hash_arr =hash_arr[1:]
    return (hash_arr.index(3)+1,hash_arr.index(0)+1)
    pass

print(rep_mis([1,2,2,3,5]))
